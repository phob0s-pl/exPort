...
const testDataCount = 1632

type PublisherMock struct {
	ProcessedCount int
}

func (p *PublisherMock) OnPort(*domain.Port) {
	p.ProcessedCount++
}

func TestProcessJSON(t *testing.T) {
	mockedPublisher := &PublisherMock{}
	if err := processJSONStream(bytes.NewBufferString(testData), mockedPublisher.OnPort); err != nil {
		t.Fatalf("failed to process stream: %s", err)
	}

	if mockedPublisher.ProcessedCount != testDataCount {
		t.Errorf("expected to process %d samples, got %d", testDataCount, mockedPublisher.ProcessedCount)
	}
}
